        ...
        try {
            ...
            if (habitArr != null) {
                for (i in 0 until habitArr.length()) {
                    ...
                    habitsList.add(
                        SuggestedHabitItem(
                            name = h.optString("name", "Daily Focus Block"),
                            targetValue = h.optDouble("targetValue", 30.0).toFloat(),
                            unit = h.optString("unit", "min"),
                            iconName = h.optString("iconName", "self_improvement"),
                            isSelected = true
                        )
                    )
                }
            ...
            if (habitsList.isEmpty()) {
                habitsList.add(SuggestedHabitItem("Daily Focus Block", 45f, "min", "self_improvement", true))
                habitsList.add(SuggestedHabitItem("Daily Hydration", 2.5f, "L", "water_drop", true))
            }

            var goalItem: SuggestedGoalItem? = null
            val goalObj = json.optJSONObject("suggestedStarterGoal")
            if (goalObj != null) {
                goalItem = SuggestedGoalItem(
                    title = goalObj.optString("title", "Establish Core Daily Routine"),
                    domain = goalObj.optString("domain", "Growth"),
                    horizon = goalObj.optString("horizon", "Quarterly"),
                    firstMilestoneTitle = goalObj.optString("firstMilestoneTitle", "Define Weekly Target Blocks"),
                    firstMilestoneDesc = goalObj.optString("firstMilestoneDesc", "Set up recurring focus times for high-impact priorities."),
                    isSelected = true
                )
            }
